...
class SerialOrchestrator(BaseWorker, ReconnectingMixin):
    def setup(self):
        try:
            ...
            logger.exception("SerialOrchestrator setup error")
            self.is_set_up = False
    
    def __init__(self):
        super().__init__(name="SerialOrchestrator")
        logger.info("SerialOrchestrator initialization 1/2")

        self.is_set_up = False

        self.setup()

        logger.info("SerialOrchestrator initialization 2/2")
    ...
    def connect(self) -> bool:
        ...
        try:
            logger.info("SerialOrchestrator connecting... 1/2")

            if self.ser is not None:
                try:
                    self.ser.close()
                except Exception as e:
                    logger.exception("SerialOrchestrator existing connection close error")
                    self.ser = None
                    return False
            ...
            try:
                ...
                logger.exception("SerialOrchestrator port existence check error")
                return False
            ...
            try:
                ...
                logger.exception("SerialOrchestrator connection error")
                self.ser = None
                return False
            ...
            if self.ser and self.ser.is_open:
                ...
                logger.info("SerialOrchestrator connected")
                return True
                
            return False
        except Exception as e:
            logger.exception("SerialOrchestrator.connect error")
            return False
    ...
    def disconnect(self) -> bool:
        if self.ser:
            try:
                ...
                logger.exception("SerialOrchestrator.disconnect close error")
                return False
    ...
    def loop(self):        
        ...
        try:
            ...
            logger.exception("SerialOrchestrator.tick instruction execution error")
            with self.queue_lock:
    ...
    def send_instruction(self, instruction: str) -> Optional[str]:
        try:
            ...
            logger.exception("SerialOrchestrator serial communication error")
            self.force_disconnect()
            return None
