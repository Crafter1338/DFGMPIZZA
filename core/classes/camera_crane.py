...
from core.classes.serial_orchestrator import _generate_move_to_instructions, _retrieve_value_from_instruction, SerialOrchestrator, static_instructions
from utility.settings import settings
from core.base_worker import BaseWorker
from core.reconnecting_mixin import ReconnectingMixin

class CameraCrane(ReconnectingMixin, BaseWorker):
    def __init__(self, serial_orchestrator: SerialOrchestrator):
        super().__init__(name="CameraCrane")
    ...
    def _null(self): # Kalibrationsfahrt des Schwenkarms
        ...
        self.serial_orchestrator.queue_instructions(static_instructions["arm"]["down"])
        ...
        self.serial_orchestrator.queue_instructions(static_instructions["arm"]["end"])
    ...
    def move_up(self):
        ...
        self.serial_orchestrator.queue_instructions(static_instructions["arm"]["end"])
        self.serial_orchestrator.queue_instructions(static_instructions["arm"]["up"])
    ...
    def move_to(self, pos: float):
        ...
        self.serial_orchestrator.queue_instructions(_generate_move_to_instructions(target_position)["SET"])
    ...
    def loop(self):
        ...
        if self.serial_orchestrator is None or not self.serial_orchestrator.is_connected() or not self.serial_orchestrator.is_set_up:
            self.position: float = 0
            self.target_position: float = 0
            self.int_target_position: int = 0
        ...
        try:
            ...
            logger.exception("CameraCrane.loop position query error")
            return
